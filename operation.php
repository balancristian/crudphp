...
function textboxValue($value){
    $textbox = mysqli_escape_string($GLOBALS['con'],trim($_POST[$value]));
    if(empty($textbox)){
        return false;
    }
    else{
...
function UpdateData(){
   ...
   if($bookname && $bookpublisher && $bookprice){
       ...
       if(mysqli_query($GLOBALS['con'], $sql)){
           ...
           TextNode("error", "Enable to Update date");
       }
   }else{
     TextNode("error", "Select data using button");
   }
}
function deleteRecord(){
    ...
    if(mysqli_query($GLOBALS['con'], $sql)){
        ...
        TextNode("error","Enabled to Delete");
    }
